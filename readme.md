...
12. 基础与初步理解
    - 如果一个前端项目使用 webpack 作为打包工具，其实从本质上来说，这个项目是一个 node 端的项目
      - 我们在项目里写的 html、js、css 文件，对于 webpack 来说，只是一堆模板，模板里面全是字符串
      - 而为了让这些字符串能在浏览器上运行，于是 webpack 就对这些文件进行编译解析，最终生成了浏览器可以识别的文件
      - 而且在开发环境，webpack 还开了一个代理服务器用来部署这些资源，以供浏览器可以正常访问
    - 何为构建工具？或者说构建工具做了啥？
      - 核心功能：
...
16. webpack 编译结果分析
    ...
    - 可以到 `./compiler/_main.explain.js` 文件内查看编译结果分析（里面的注释写的很详细哈，重点看这个文件！[转至此文件](./compiler/_main.explain.js)）
      ...
      - 至于 webpack 是怎么做代码转换的？
        - 大致流程就是：
          - webpack 会先读 JS 文件（一开始读入口 JS 文件），得到的全是字符串
          - 然后利用 AST 工具将其转化为抽象语法树，做语法分析，进行文件内容替换
            - 例如将 `import 'xxx'` 替换为 `__webpack_require__('xxx')`，其实就是纯字符串文本替换，简单写，使用 `replace` 方法即可实现
          - 最后把处理后的文件输出出来
        - webpack 内部使用的 AST 分析工具也是 babel 提供的
        - 具体执行流程后面再说
