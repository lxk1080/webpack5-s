 ...
 *          - 但其实，它并不单纯，它并没有在再次构建时简单直接的清空 dist 目录
 ...
 *          - 要解决这个问题，目前有两个方法：
 *            - 1、自己写个清空 dist 插件，确保每次构建是真的清空文件，这样子 webpack 就不得不重建文件了
 *                - 重建后的文件就是包含最新内容的文件了，但是 hash 值还是没有改变的哈，只是重建
 *            - 2、不要使用各种 hash 值去命名文件，因为 webpack 会根据 hash 去判断文件内容有没有变化，从而决定要不要清除原文件
 *                - 你如果都没有 hash 值，webpack 就弄不清楚文件内容到底变没变，为了确保无误，它只能选择重建文件
 *                - 但是这个方式不太现实，我们上生产需要依赖 hash 值做缓存，是不可能不用 hash 的，所以最好还是写插件解决吧
 *          - 当然正确的解决方法应该是在插件中修改，修改文件内容的同时，同步修改 hash 值，但这个目前做不到。。
 */
